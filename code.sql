...
------------------------------------------wasim-------------------
      IF (r_gggs.data_type = k_customer) THEN
        ...
        IF (r_gggs.process_type = k_new) THEN
          INSERT INTO gggs_customer (CUSTOMERID, NAME, PROVINCE, FIRST_NAME, LAST_NAME, CITY, PHONE_NUMBER, STATUS)
     ...
     SET province     = CASE WHEN r_gggs.column2 IS NULL OR r_gggs.column2 = k_no_change_char
                             THEN gc.province ELSE r_gggs.column2 END,
         first_name   = CASE WHEN r_gggs.column3 IS NULL OR r_gggs.column3 = k_no_change_char
                             THEN gc.first_name ELSE r_gggs.column3 END,
         last_name    = CASE WHEN r_gggs.column4 IS NULL OR r_gggs.column4 = k_no_change_char
                             THEN gc.last_name ELSE r_gggs.column4 END,
         city         = CASE WHEN r_gggs.column5 IS NULL OR r_gggs.column5 = k_no_change_char
                             THEN gc.city ELSE r_gggs.column5 END,
         phone_number = CASE WHEN r_gggs.column6 IS NULL OR r_gggs.column6 = k_no_change_char
                             THEN gc.phone_number ELSE r_gggs.column6 END
